...
export interface OrbSceneApi {
  ...
  zoomOut(): void;
  resetView(): void;
  setOrbMode(mode: 'standby' | 'listening' | 'thinking' | 'speaking' | 'adjusting'): void;
  dispose(): void;
}
...
export function createOrbScene(container: HTMLElement): OrbSceneApi {
  ...
  const targetColorBright = new THREE.Color(PALETTES.standby.bright);
  const targetColorMid = new THREE.Color(PALETTES.standby.mid);
  const targetColorDim = new THREE.Color(PALETTES.standby.dim);
  const targetColorFaint = new THREE.Color(PALETTES.standby.faint);
  const targetColorHot = new THREE.Color(PALETTES.standby.hot);

  const colorBright = targetColorBright.clone();
  const colorMid = targetColorMid.clone();
  const colorDim = targetColorDim.clone();
  const colorFaint = targetColorFaint.clone();
  const colorHot = targetColorHot.clone();

  let targetSpeedScale = 1.0;
  let speedScale = 1.0;
  let currentMode: 'standby' | 'listening' | 'thinking' | 'speaking' | 'adjusting' = 'standby';
  ...
  for (let i = -15; i <= 15; i++) {
    ...
    const color = i % 3 === 0 ? colorMid : colorFaint;
  ...
  for (let i = 0; i < 24; i++) {
    ...
    outerShell.add(
      new THREE.Line(
        meridian(R1, lon),
        lineMat(isMajor ? colorMid : colorFaint, isMajor ? 0.6 : 0.1),
      ),
    );
  }
  ...
  for (let i = 0; i < 4; i++) {
    ...
    for (let j = 0; j < CROSS_LINES; j++) {
      ...
      const color = Math.abs(t) < 0.3 ? colorBright : colorMid;
      outerShell.add(
  ...
  function createSpherePanel(
    ...
    const mat = lineMat(colorDim, 0.25);
  ...
  for (let i = 0; i < 16; i++) {
    ...
    shell2.add(
      new THREE.Line(
        ...
        lineMat(colorMid, 0.2 + Math.random() * 0.3),
      ),
    );
  }
  ...
  for (let i = 0; i < 12; i++) {
    ...
    shell2.add(
      new THREE.Line(
        new THREE.BufferGeometry().setFromPoints(pts),
        lineMat(colorDim, 0.15 + Math.random() * 0.2),
      ),
    );
  }
  orbGroup.add(shell2);
  ...
  for (let s = 0; s < 8; s++) {
    ...
    innerCore.add(
      new THREE.Line(
        new THREE.BufferGeometry().setFromPoints(pts),
        lineMat(colorBright, 0.3 + Math.random() * 0.2),
      ),
    );
  }
  ...
  for (let i = -6; i <= 6; i++) {
    ...
    innerCore.add(new THREE.Line(latRing(R3, lat, 80), lineMat(colorDim, 0.2)));
  }

  // Inner meridians
  for (let i = 0; i < 12; i++) {
    const lon = (i / 12) * Math.PI * 2;
    innerCore.add(new THREE.Line(meridian(R3, lon, 80), lineMat(colorDim, 0.15)));
  }
  ...
  const icoWireMat = lineMat(colorHot, 0.9);
  ...
  function makeTextSprite(text: string, size = 0.08, spriteColor: THREE.Color = colorBright) {
  ...
  function scatterText(count: number, sizeFn: () => number, rFn: () => number, speedScale: [number, number], spriteColor: THREE.Color) {
    const group = new THREE.Group();
  ...
  for (let i = 0; i < 250; i++) {
    ...
    const mat = new THREE.MeshBasicMaterial({
      color: Math.random() > 0.7 ? colorBright : colorMid,
      transparent: true,
  ...
  // Soft dot texture - white, tinted by dustMat.color
  ...
  for (let i = 0; i < 15; i++) {
    ...
    const hex = new THREE.LineSegments(hexEdges, lineMat(colorMid, 0.5));
    hex.position.set(
      r * Math.sin(phi) * Math.cos(theta),
      r * Math.cos(phi),
      r * Math.sin(phi) * Math.sin(theta),
  ...
  function animate() {
    ...
    // Lerp colors for smooth shifting
    const lerpSpeed = 0.06;
    colorBright.lerp(targetColorBright, lerpSpeed);
    colorMid.lerp(targetColorMid, lerpSpeed);
    colorDim.lerp(targetColorDim, lerpSpeed);
    colorFaint.lerp(targetColorFaint, lerpSpeed);
    colorHot.lerp(targetColorHot, lerpSpeed);

    // Dynamic points color update
    dustMat.color.copy(colorBright);

    // Lerp speed scale
    speedScale += (targetSpeedScale - speedScale) * 0.06;

    // Outer shell rotation
    outerShell.rotation.y += 0.0015 * speedScale;
    outerShell.rotation.x = Math.sin(t * 0.08) * 0.05;

    // Panel group follows shell but with slight offset
    panelGroup.rotation.y += 0.0018 * speedScale;
    panelGroup.rotation.x = Math.sin(t * 0.08 + 0.5) * 0.04;

    // Secondary shell counter-rotates slowly
    shell2.rotation.y -= 0.001 * speedScale;
    shell2.rotation.z = Math.sin(t * 0.12) * 0.03;

    // Inner core — opposite, faster
    innerCore.rotation.y -= 0.005 * speedScale;
    innerCore.rotation.z += 0.002 * speedScale;
    innerCore.rotation.x = Math.cos(t * 0.1) * 0.08;

    // Innermost wireframe
    icoWire.rotation.x += 0.008 * speedScale;
    icoWire.rotation.y += 0.012 * speedScale;
    ...
    debris.forEach((d) => {
      ...
      const a = t * u.speed * speedScale + u.phase;
      d.position.set(
    ...
    for (const [group, mult] of driftGroups) {
      group.children.forEach((sp) => {
        ...
        u.theta += u.speed * mult * speedScale;
        sp.position.set(
    ...
    // Dust rotation
    dustPoints.rotation.y += 0.0002 * speedScale;
    ...
    // Bloom pulse base strength and speed scaling by mode
    const pulseSpeed = currentMode === 'thinking' ? 4.5 : currentMode === 'listening' ? 2.5 : currentMode === 'adjusting' ? 3.5 : 0.8;
    const baseStrength = currentMode === 'thinking' ? 2.3 : currentMode === 'listening' ? 1.9 : currentMode === 'adjusting' ? 2.1 : 1.6;
    bloom.strength = baseStrength + Math.sin(t * pulseSpeed) * 0.4;
  ...
  function setOrbMode(mode: 'standby' | 'listening' | 'thinking' | 'speaking' | 'adjusting') {
    currentMode = mode;
    const palette = PALETTES[mode] || PALETTES.standby;
    targetColorBright.setHex(palette.bright);
    targetColorMid.setHex(palette.mid);
    targetColorDim.setHex(palette.dim);
    targetColorFaint.setHex(palette.faint);
    targetColorHot.setHex(palette.hot);

    if (mode === 'standby') {
      targetSpeedScale = 1.0;
    } else if (mode === 'listening') {
      targetSpeedScale = 2.2;
    } else if (mode === 'thinking') {
      targetSpeedScale = 4.5;
    } else if (mode === 'speaking') {
      targetSpeedScale = 1.5;
    } else if (mode === 'adjusting') {
      targetSpeedScale = 3.0;
    }
  }
